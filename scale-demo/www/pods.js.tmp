...
var image = "quay.io/aleks_saul/hello_world";
...
PodsApp.prototype.getVersionColor = function(pod) {
    if (pod.spec.containers[0].image == image + ":production-10") {
	return "#3F51B5";
    }
    if (pod.spec.containers[0].image == image + ":production-11") {
	return "#C5CAE9";
    }
    return "#DDDDDD";
};
...
PodsApp.prototype.refresh = function() {
    ...
    var promises = [];
    promises.push(this.http.get("/api/v1/pods?labelSelector=app=helloworld")
